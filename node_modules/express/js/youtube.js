...
function onYouTubeIframeAPIReady() {
  loadVideos().always(function() { //is returned as deffered object
    ...
    setInterval(function(){
      try{
        YTCache.set('lastTime', getYTCurrentTime(curr) + '', true);
      }catch(err){
        console.log(err);
      }
    }, 1000);
...
function YTError(ytplayer){
  ...
  if(ytplayer == curr){
    //alert('error curr');
    tag = currdisp.substring(1,currdisp.length);
    videoId = queue[currindex];
    if(currready){
      curr.getIframe().setAttribute("src", "https://www.youtube.com/watch?v=WsksFbFZeeU");
      currready = false;
    }
